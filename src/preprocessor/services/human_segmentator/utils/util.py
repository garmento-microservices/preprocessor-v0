...
def cross_entropy2d_dataparallel(
    logit, target, ignore_index=255, weight=None, size_average=True, batch_average=True
):
    n, c, h, w = logit.size()
    # logit = logit.permute(0, 2, 3, 1)
...
def scale_tensor_list(
    ...
    for i in range(len(input) - 1):
        ...
        for j in range(len(input[i])):
            ...
            output_item.append(
                F.upsample(
                    input[i][j], size=(h, w), mode="bilinear", align_corners=True
                )
            )
...
def scale_tensor_list_0(input, base_input):
    ...
    for j in range(len(input)):
        ...
        after_size = F.upsample(
            input[j], size=(h, w), mode="bilinear", align_corners=True
        )
